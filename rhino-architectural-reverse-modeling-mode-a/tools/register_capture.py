...
def main():
    ...
    print("raw capture      : %dx%d" % size)
    print("building extent  : x %d..%d  width %d  top %d" % (x_lo, x_hi, bw, top))
    print("expected width   : %.1f" % a.expect_width)
    print("lens correction  : %.5f" % factor)

    if abs(bw - a.expect_width) > a.tolerance:
        print()
        print("*** SCALE DRIFT — %.1f px off. Do NOT crop. ***" % (bw - a.expect_width))
        print("The viewport pixel size or the capture width changed. Multiply the")
        print("lens by %.5f, re-capture at the SAME requested size, and re-run." % factor)
        return 1

    cx, cy = int(round(x_lo - a.ref_x)), int(round(top - a.ref_y))
    if cx < 0 or cy < 0 or cx + a.width > size[0] or cy + a.height > size[1]:
        print()
        print("*** CROP OUT OF BOUNDS — capture a larger frame. ***")
        print("needed origin (%d,%d) size %dx%d from %dx%d" % (cx, cy, a.width, a.height, size[0], size[1]))
        return 1

    im.crop((cx, cy, cx + a.width, cy + a.height)).save(a.out)
    print("crop offset      : (%d, %d)  ->  %s" % (cx, cy, a.out))
    return 0
